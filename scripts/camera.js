...
export function cameraPosition(camera, player, map, canvas, number_level) {
    ...
    let camerYoff = 10.0;
    let lagFactor = 0.05;
    if (map.levels[number_level].camera_position != undefined) {
        if (map.levels[number_level].camera_position == "player"&&camera_object == undefined) {
            let dx,dy;
            dx = player.x+50 - camera.x + mouse.x/8;
            dy = player.y- camerYoff - camera.y + mouse.y/10;
            if (player == undefined) {
                camera.x = 350
            }else{
        ...
        else if(map.levels[number_level].camera_position == "player"&&camera_object != undefined){
        ...
        }else if (map.levels[number_level].camera_position == "default"||map.levels[number_level].camera_position == undefined) {
        ...
        if (map.camera_position == "player"&&camera_object == undefined) {
            ...
            player.forEach(player1 => {
                dx = player1.x+50 - camera.x + mouse.x/8;
                dy = player1.y- camerYoff - camera.y + mouse.y/10;
            });
            camera.x += dx * lagFactor;
            camera.y += (dy * lagFactor) / 2;
        }
