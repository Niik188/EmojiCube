...
export function tile_functional(player, map, json, difficulty, gun) {
    ...
    cubes.forEach((cube) => {
        ...
        if (cube.mouse.pressing() && distance < 250) {
            ...
            if (cube.collider == "dynamic" && cube.colliding(tiles) && !cube.colliding(jumping) && !cube.colliding(speedRight) && !cube.colliding(speedLeft)) {
                setTimeout(() => {
                    cube.collider = "k";
                    cube.sleeping = true;
                    cube.active = false;
...
export function chanceColorTiles(level) {
    ...
    tiles.forEach((tile, index) => {
        if (level.tile_option != undefined && level.tile_option[index + 1] != undefined && level.tile_option[index + 1].img == undefined) {
