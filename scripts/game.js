...
import { controls, p5controls, god_mode} from "./controls.js";
...
import { boss, boss_ready, chanceColorTiles, emoji, load_tiles, objects, scoreDeaths, setup_tiles, spawns, tiles, tile_functional, win } from "./tiles.js";
import { getRandomInt } from "./utils.js";
export let player
let player_spawn = { active: false, x: 0, y: 0 };
let playerTextdefult = '😐'
let playerColordefult = '#FFC83D'
let gun;export let bullets;
let difficulty;
...
function setup() {
    ...
    player.emojis = {
        win:'😄',schock:'😲',unruhe:['😟','😨','😱'],
        win_color:'#FFC83D',schock_color:'#FFC83D',unruhe_color:['#FFC83D','#FFC83D','#FFC83D']
    }
    player.death = false
    ...
    if (random_count <= 3&&random_count >= 2) {
        ...
        player.emojis = {
            win:'🥵',schock:'😳',unruhe:['🤨','😳','🥵'],
            win_color:'#F03A17',schock_color:'#FFC83D',unruhe_color:['#FFC83D','#FFC83D','#F03A17']
        }
    }else if (random_count >= 6&&random_count <= 8){
        document.title = '🤓Cube'
        playerTextdefult = '☝️🤓ᅟ'
        player.emojis = {
            win:'📙😄ᅟ',schock:'🖐️😲🖐️',unruhe:['☝️😟ᅟ','☝️😨ᅟ','😱'],
            win_color:'#FFC83D',schock_color:'#FFC83D',unruhe_color:['#FFC83D','#FFC83D','#FFC83D']
        }
        ...
        player.emojis = {
            win:' ͡° ͜ʖ ͡°',schock:'｡>﹏<｡',unruhe:['ಠ_ಠ','ŏ﹏ŏ','ó_ò'],
            win_color:'white',schock_color:'white',unruhe_color:['white','white','white']
        }
    }
...
export function playerSetTextDefult(timer) {
...
export function map_create(restart_level, death) {
    background(0)
    tiles.removeAll(); objects.removeAll()
    ...
    if (map.levels[random_level] == map.levels[0]) {map.begin_level = false}
    player.rotationLock = map.levels[random_level].rotationLock;
    if (map.player_ball) {player.diameter = 30}else{player.w = 38;player.h = 38}
    if (map.gun_of_win) {
        win.text = '🔫'
        win.w = 10
        win.h = 10
    ...
    world.gravity.x = 0; world.gravity.y = 10;
    if (map.gravity!=undefined) {
        world.gravity.x = map.gravity.x; world.gravity.y = map.gravity.y;
    }
    ...
    } catch (error) {player.x = player_spawn.x; player.y = player_spawn.y}
